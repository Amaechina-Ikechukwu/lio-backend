...
(req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Destructure 'uid' directly from req.body
        const { userprofile } = yield (0, GetUserProfile_1.default)(req.uid);
        const data = Object.assign(req.body, {
            user: userprofile.displayName.trim().toLowerCase().split(" ").join("-"),
        });
        const result = yield (0, CreatePortfolio_1.default)(data, req.uid); // Pass 'uid' directly to RegisterUser function
        res.status(200).json({ token: result });
    }
    catch (error) {
        console.error("Add user to database user", error);
