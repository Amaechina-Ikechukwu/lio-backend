...
router.get("/auth/google", (req, res) => {
    ...
    <body>
      <button id="googleSignInButton" class="bg-black text-gray-200 rounded-xl shadow-xl py-4 px-4 font-bold transition duration-300 transform hover:scale-105 hover:ring-light-accent focus:outline-none ring ring-gray-300">
        Sign in with Google
      </button>
      ...
      <script>
        document.addEventListener('DOMContentLoaded', function() {
          ...
          googleSignInButton.addEventListener('click', async function() {
            try {
              const { GoogleAuthProvider } = require("firebase/auth"); // tslint:disable-line:no-unused-variable
const { signInWithPopup } = require("firebase/auth"); // tslint:disable-line:no-unused-variable
const { getAuth } = require("firebase/auth"); // tslint:disable-line:no-unused-variable
              const auth = getAuth()
